...
serve(async (req) => {
  ...
  try {
    ...
    const tx_ref = `VER_${user_id}_${Date.now()}`;
    ...
    const response = await fetch('https://api.flutterwave.com/v3/payments', {
      ...
      body: JSON.stringify({
        ...
        amount,
        currency: 'NGN',
        redirect_url: `${req.headers.get('origin') || 'https://singlez.lovable.app'}/verification?payment=complete`,
        customer: {
          email: user_email,
        },
        customizations: {
          title: 'SinglezConnect Verification',
          description: `${plan_type} verification badge`,
